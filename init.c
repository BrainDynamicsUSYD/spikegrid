...
model* setup(const parameters p,const parameters p2,const LayerNumbers lcount)
{
    ...
    output_s* outdata=(output_s[]){ //note - neat feature - missing elements initailized to 0
        {.name="gE",.data={m2->gE,conductance_array_size,couplerange},.minval=0,.maxval=2}, //gE is a 'large' matrix - as it wraps around the edges
        {"gI",{m2->gI,conductance_array_size,couplerange},0,1}, //gI is a 'large' matrix - as it wraps around the edges
